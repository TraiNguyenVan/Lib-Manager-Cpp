...
long Date::toDayCount() const {
    long total = 0;
    for (int y = 0; y < year; y++) total += (isLeapYear(y)) ? 366 : 365;
...
bool Date::operator<(const Date& other) const { return toDayCount() < other.toDayCount(); }
...
Date Date::operator+(int daysToAdd) const { return fromDayCount(toDayCount() + daysToAdd); }

Date Date::operator-(int daysToSubtract) const {
    return fromDayCount(toDayCount() - daysToSubtract);
}

long Date::operator-(const Date& other) const { return toDayCount() - other.toDayCount(); }
std::ostream& operator<<(std::ostream& os, const Date& d) {
    os << (d.day < 10 ? "0" : "") << d.day << "/" << (d.month < 10 ? "0" : "") << d.month << "/"
       << d.year;
    return os;
}
